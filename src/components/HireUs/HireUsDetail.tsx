...
const HireUsDetail: React.FC<HireUsCardProps> = ({ id, title, description}) => {
  return (
    <section className="font-roboto tracking-wider flex px-4 flex-shrink-0 flex-grow-0 w-1/2 justify-center">
      <RiseFromBelow>
        <h3 className={`text-3xl font-black text-center mt-2`}>{description}</h3>
      </RiseFromBelow>
    </section>
  );
};
